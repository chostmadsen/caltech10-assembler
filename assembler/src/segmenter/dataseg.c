...
[[nodiscard]] static bool data_chck_add_( strptr *const sptr,
    ...
    cit10a_asrt(sptr != nullptr && smap_v != nullptr);
    cit10a_asrt(*loc >= 0);
    ...
    data_chck_dup_(&smap_itm, (stackmap*)smap_v, err_f);
    const   bool    ret =   identifier_verify((stackmap*)smap_v, &smap_itm, err_f);
    ++(*loc);
    return  ret;
}
...
[[nodiscard]] bool dataseg(       int      *const org,
    ...
    return  commonseg(org, source, srcs, tok_data, data_chck_add_, smap);
}
